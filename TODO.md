...
- **Wire up `agentrex.aeo-rex.com` subdomain** — currently live at `peppy-alpaca-efe75b.netlify.app`. Steps: add CNAME record at the DNS provider pointing `agentrex` to the Netlify site, add the custom domain in Netlify site settings, wait for the auto-issued Let's Encrypt cert. Once live, update the README "Live demo" line and the canonical/og:url tags in `index.html` will already point at the right place.
