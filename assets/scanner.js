...
/* ---------- Utility: fetch via proxy with timeout ----------
   Returns { ok, status, text, reason } where reason is one of:
     "ok"            — 2xx response from upstream
     "http_error"    — 4xx/5xx from upstream (resource doesn't exist / blocked)
     "rate_limited"  — 429
     "proxy_error"   — 502/504 from our function (couldn't reach upstream)
...
async function safeFetch(url, timeoutMs = 7000) {
  ...
  try {
    ...
    if (r.status === 502 || r.status === 504) {
      return { ok: false, status: r.status, text: "", reason: "proxy_error" };
    }
...
function proxyErrorDetail(reason, what) {
  const phrase = reason === "rate_limited"  ? "rate-limited"
              : reason === "proxy_error"    ? "proxy couldn't reach the site"
...
function verdictInconclusive(unknownCount) {
  return {
    ...
    sub: `${unknownCount} of ${CHECKS.length} checks were inconclusive — the proxy couldn't reach the site, was rate-limited, or timed out. Try again in a minute. If the same checks keep failing, your site may be down or behind aggressive bot protection that also blocks AI agents — which is itself an agent-readiness problem worth fixing.`
  };
}
